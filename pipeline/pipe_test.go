...
func TestPipe_Read(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			p:    New(16, []byte{0x01, 0x02, 0x03, 0x04, 0x05}),
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !compare.Equal(n1, n2) || !compare.Equal(err1, err2) || !compare.Equal(data1, data2) {
...
func TestPipe_Write(t *testing.T) {
	t.Run("write error", func(t *testing.T) {
		...
		n2, err2 := p.Read(b)
		b = b[:n2]
		if n != 0 || !compare.Equal(err, errwrite) || n2 != 1 || err2 != nil || !compare.Equal(b, []byte{0x01}) {
